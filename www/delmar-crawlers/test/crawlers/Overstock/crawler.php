...
// Model_no
            $item_model = $html->find('#details_descMisc');

            if (!empty($item_model)) {
                $details = $item_model[0]->innertext;
                preg_match("/.*Model No:<\/dt>\s*<dd>(.*)<\/dd>/", $details,  $model_matches);
                $model = '';
                if (!empty($model_matches)) {
                    $model = trim($model_matches[1]);
                }
//                             echo "\n Descr-->" . trim($description_item[0]->plaintext);
                $data['ad']['model_no'] = $model;
                phpCrawler::log()->logInfo("MODEL'KA NEW ". $data['ad']['model_no'] . "\nDETAILS");
                phpCrawler::log()->logInfo($details);
                phpCrawler::log()->logInfo("*** model_matches ***");
                phpCrawler::log()->logInfo($model_matches);
                
 //Materials               
                preg_match("/.*Materials:<\/dt>\s*<dd>(.*)<\/dd>/", $details,  $material_matches);
                $material = '';
                if (!empty($material_matches)) {
                    $material = trim($material_matches[1]);
                }
                $data['ad']['AD_Materials'] = $material;
                phpCrawler::log()->logInfo("Materials NEW". $data['ad']['AD_Materials'] . "\nDETAILS");
                phpCrawler::log()->logInfo($details);
                phpCrawler::log()->logInfo("*** material_matches ***");
                phpCrawler::log()->logInfo($material_matches);
            }


// Item ID
            $id_item = $html->find('.prod-item-number');
            if (!empty($id_item)) {
                $item_no = $id_item[0]->plaintext;
                preg_match("/.*?(\d+)/", $item_no,  $id_matches);
                $item_num = '';
                if (!empty($id_matches)) {
                    $item_num = trim($id_matches[1]);
                }
//                             echo "\n Descr-->" . trim($description_item[0]->plaintext);
                $data['ad']['item_no'] = $item_num;
            }

// Category
            $item_category = $html->find('.crumb-4 a');
            if (!empty($item_category)) {
                $item_cat = $item_category[0]->plaintext;
                $item_categ = trim($item_cat);
//           echo "\n Descr-->" . trim($description_item[0]->plaintext);
                $data['ad']['category'] = $item_categ;
            }

// Subcategory
            $item_subcategory = $html->find('.crumb-5 a');
           if (!empty($item_subcategory)) {
                $item_subcat = $item_subcategory[0]->plaintext;
                $item_subcateg = trim($item_subcat);
//              echo "\n Descr-->" . trim($description_item[0]->plaintext);
                $data['ad']['subcategory'] = $item_subcateg;
            }
