...
class tools_importer {
	...
	$TABLE = ORM::for_table($this->_sourceDataTable)->select_expr("COUNT(id)","count")->find_one();
	phpCrawler::log()->logDebug("= = = >" . $TABLE->count);
        if ($TABLE->count <= 0) {
            phpCrawler::log()->logDebug("DATABASE " . $this->_sourceDataTable . " IS EMPTY");
            return null;
        }
        else {
	    $maxId = ORM::for_table($this->_sourceDataTable)->select_expr("MAX(id)","max")->find_one();
            phpCrawler::log()->logDebug("MAX ID = " . $maxId->max);
        }
        ...
        while ($need) {
		    ...
		    $row["AD_Title"] = $this->cl($ad->AD_Title);
                    ...
                    $row["AD_PostDate"] = $this->cl($ad->AD_PostDate);
 		    ...
 		    $row["AD_ContactLink"] = 'NULL';
                    ...
                    $row["app_Typeindex"] = $this->cl($ad->app_Typeindex);
                    $row["app_Rooms"] = $this->num($ad->app_Rooms);
		    $row["app_BathRooms"] = $this->num($ad->app_BathRooms);
                    $row["app_Floor"] = $this->cl($ad->app_Floor);
                    ...
                    $row["app_PartnerSex"] = $this->cl($ad->app_PartnerSex);
                    ...
                    $row["AD_Addition"] = $this->cl($ad->car_Addition);
